...
export default function NuestroEquipoPage() {
  return (
    <section className="bg-surface py-10 md:py-14">
      <Container>
        <Reveal>
          <h1 className="text-center font-serif text-3xl font-semibold tracking-tight text-navy md:text-4xl">
            Nuestro equipo
          </h1>
        </Reveal>
      </Container>
        ...
        {teamMembers.map((member, index) => {
          ...
          return (
            <Reveal key={member.id} delay={index * 0.08}>
              <article className="overflow-hidden bg-navy md:rounded-2xl">
                <div className="relative flex flex-col md:min-h-[36rem] lg:min-h-[40rem] xl:min-h-[44rem]">
                  ...
                  <div
                    className={cn(
                      "relative z-10 flex w-full items-center justify-center bg-navy px-7 py-10 text-cream md:absolute md:inset-y-0 md:w-[52%] md:px-10 lg:px-12",
                      imageOnLeft
                        ? "md:right-0 md:[clip-path:polygon(4%_0,100%_0,100%_100%,0_100%)]"
                  ...
                  >
                    <div className="mx-auto w-full max-w-md text-center">
                      ...
                      <p className="mt-2 text-sm font-medium text-cream/80">{member.role}</p>
                      <ul className="mt-6 space-y-3 text-left text-sm leading-7 text-cream/85">
                        {member.highlights.map((item) => (
                          <li key={item} className="flex gap-3">
                            <span
                              className="mt-2.5 h-1.5 w-1.5 shrink-0 rounded-full bg-cream"
                              aria-hidden="true"
                            />
                            <span>{item}</span>
                          </li>
